...
def show_metric_selector(metrics_df: pd.DataFrame):
    ...
    sorted_metrics = sort_items(
        list(
            map(
                lambda m: m.value,
                metrics_df.sort_values("order")
                .loc[metrics_df["showing"], "metric"]
                .to_list(),
            )
        )
    )
    ...
    if (
        not temp["order"].is_monotonic_increasing
        and not temp["order"].is_monotonic_decreasing
    ):
        index = temp.index
...
def set_groups(node_id: str):
    ...
    groups = pd.DataFrame(
        {Names.GROUPS.value: iteration._groups, "Selected": iteration._group_mask}
    )
    ...
    column_config = {
        ...
        Names.LOWER_BOUND.value: st.column_config.NumberColumn(
            label=Names.LOWER_BOUND.value, disabled=True, format="compact"
        ),
        Names.UPPER_BOUND.value: st.column_config.NumberColumn(
            label=Names.UPPER_BOUND.value, disabled=True, format="compact"
        ),
