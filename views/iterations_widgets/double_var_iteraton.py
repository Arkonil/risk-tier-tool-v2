...
def show_edited_grid(
    scalars_enabled: bool, split_view_enabled: bool, metrics: pd.Series, key: int = 0
) -> t.Optional[str]:
    ...
    if iteration.var_type == "numerical":
        grid_index[Names.LOWER_BOUND.value] = grid_index[Names.GROUPS.value].map(
            lambda bounds: bounds[0]
        )
        grid_index[Names.UPPER_BOUND.value] = grid_index[Names.GROUPS.value].map(
            lambda bounds: bounds[1]
        )
        grid_index.drop(columns=[Names.GROUPS.value], inplace=True)
    else:
        grid_index.rename(
            columns={Names.GROUPS.value: Names.CATEGORIES.value}, inplace=True
        )

    # Conttoller
    # Create the grid DataFrame
    grid_df = grid_index.merge(
        iteration.risk_tier_grid.loc[
            :, iteration_graph.get_parent_tiers(iteration.id)
        ].replace(SUB_RISK_TIERS),
        how="left",
        left_index=True,
        right_index=True,
    ...
    column_config = {
        Names.LOWER_BOUND.value: st.column_config.NumberColumn(
            label=Names.LOWER_BOUND.value, disabled=False, format="plain"
        ),
        Names.UPPER_BOUND.value: st.column_config.NumberColumn(
            label=Names.UPPER_BOUND.value, disabled=False, format="plain"
        ),
    }
    ...
    grid_df = grid_df.style.map(
        lambda v: f"background-color: {color_map[v]};", subset=selectbox_columns
    )
    ...
    edited_styled_grid = edited_grid.drop(
        columns=[
            Names.LOWER_BOUND.value,
            Names.UPPER_BOUND.value,
            Names.CATEGORIES.value,
        ],
        errors="ignore",
    ).style.map(
        lambda v: f"background-color: {color_map[v]};", subset=selectbox_columns
    ...
    col2.dataframe(
        edited_styled_grid, hide_index=True, key=f"edited_grid_{iteration.id}_1"
    )

    # Replace the values in the edited grid with the corresponding risk tier values
    with pd.option_context("future.no_silent_downcasting", True):
        edited_grid = (
            edited_grid.replace(SUB_RT_INV_MAP)
    ...
    previous_risk_tiers, errors, warnings, _ = iteration_graph.get_risk_tiers(
        iteration_graph.get_parent()
    )
    ...
    if scalars_enabled and (
        Metric.ANNL_WO_COUNT_PCT.name in showing_metric_names
        or Metric.ANNL_WO_BAL_PCT.name in showing_metric_names
    ):
        ...
        summ_df = (
            summ_df.merge(
                iteration.risk_tier_grid.stack()
                .rename_axis(
                    index=[Names.GROUP_INDEX.value, Names.RISK_TIER_VALUE.value]
                )
        ...
        summ_df = summ_df.drop(
            columns=["rsf_ulr", "rsf_dlr", "New Risk Tier"], errors="ignore"
        )

    def calculate_grid_of_metric(metric: Metric) -> pd.DataFrame:
        metric_summary = (
            summ_df[metric.value]
            .to_frame()
            .unstack(1)
            .droplevel(0, axis=1)
        ...
        metric_summary = pd.merge(
            grid_index, metric_summary, how="left", left_index=True, right_index=True
        )
        ...
        for i in metric_summary.data.index:
            for j in metric_summary.data.columns:
                ...
                color = color_map[
                    SUB_RISK_TIERS.loc[edited_grid.loc[i, SUB_RT_INV_MAP.loc[j]]]
                ]
                metric_summary = metric_summary.set_properties(
                    **{"background-color": color}, subset=idx[i, j]
                )
    ...
    selectbox_columns = (
        previous_risk_tiers.dropna().drop_duplicates().sort_values().map(SUB_RISK_TIERS)
    )
    ...
    with st.expander("Grid View", expanded=True):
        ...
        for metric in showing_metrics:
            ...
            if metric.value in [Metric.VOLUME.value, Metric.WO_COUNT.value]:
                metric_summary = metric_summary.format(
                    precision=0, thousands=",", subset=selectbox_columns
                )
            elif metric.value in [Metric.AVG_BAL.value, Metric.WO_BAL.value]:
                metric_summary = metric_summary.format(
                    precision=2, thousands=",", subset=selectbox_columns
                )
            elif metric.value in [
                ...
                metric_summary = metric_summary.format(
                    lambda v: f"{v:.2f} %", subset=selectbox_columns
                )

            new_column.markdown(f"### {metric.value}")
            new_column.dataframe(
                metric_summary,
                hide_index=True,
                column_config=column_config,
                use_container_width=True,
...
def show_double_var_iteration_widgets():
    ...
    with st.sidebar:
        ...
        if col2.button(
            "", use_container_width=True, icon=":material/add:", key="Add Group"
        ):
