...
def streamlit_flow_graph(iteration_graph: IterationGraph) -> StreamlitFlowState:
    ...
    selected_node_id = iteration_graph.current_selected_node_id
    ...
    for node_id, iteration in iterations.items():
        ...
        if iteration_graph.iteration_depth(node_id) == 1:
            node_type = 'input'
        elif iteration_graph.iteration_depth(node_id) == 2:
            node_type = 'default'
        else:
            node_type = 'output'
        ...
        nodes.append(StreamlitFlowNode(
            id=f"{node_id}",
            pos=(100, 100),
            data={'content': f'<p><strong>Iteration #{node_id}</strong><br /><i>{iteration.variable.name}</i></p>'},
            node_type=node_type,
...
def show_primary_iter_var_selection_dialog():
    ...
    if st.button("Select"):
        ...
        session.iteration_graph.add_single_var_node(variable)
        st.rerun()
...
def show_secondary_iter_var_selection_dialog(parent_node_id: str):
    ...
    if st.button("Select"):
        variable = data.load_column(variable_name)
        session.iteration_graph.add_double_var_node(parent_node_id, variable)
        st.rerun()
...
def show_iteration_graph_widgets():
    ...
    if new_state.selected_id is not None:
        if st.sidebar.button(f"Open Iteration #{new_state.selected_id}", use_container_width=True, type="primary", icon=":material/open_in_new:"):
            iteration_graph.select_viewing_node_id(new_state.selected_id)
            st.rerun()
    
    if new_state.selected_id is None:
        if st.sidebar.button("Add New Iteration", icon=":material/add:", use_container_width=True, type="secondary"):
            show_primary_iter_var_selection_dialog()
    else:
        if iteration_graph.iteration_depth(new_state.selected_id) < 3:
            if st.sidebar.button("Add Child Iteration", icon=":material/add:", use_container_width=True, type="secondary"):
                show_secondary_iter_var_selection_dialog(new_state.selected_id)
                
        if st.sidebar.button("Delete Iteration", icon=":material/delete:", use_container_width=True, type="secondary"):
