...
def show_data_importer_page():
    ...
    with st.container(border=True, key="final_application_status"):
        ...
        data.var_app_status = st.selectbox(label="Application Status", options=data.sample_df.columns, index=data.get_col_pos(data.var_app_status), label_visibility="collapsed")
        ...
        if data.var_app_status is not None:
            ...
            with col2:
                if data.var_app_status_declined:
                    selected_vals = st.multiselect("Declined", options=data.var_app_status_declined, disabled=True, default=data.var_app_status_declined)
                else:
        ...
        with col1:
            ...
            data.var_unt_wrt_off = st.selectbox("#WO", options=data.sample_df.columns, index=data.get_col_pos(data.var_unt_wrt_off), label_visibility="collapsed")
        
        with col2:
            st.markdown("###### $ Write Off Balance")
            data.var_dlr_wrt_off = st.selectbox("$WO", options=data.sample_df.columns, index=data.get_col_pos(data.var_dlr_wrt_off), label_visibility="collapsed")
        
        with col3:
            st.markdown("###### Average Recievable")
            data.var_avg_bal = st.selectbox("avg_bal", options=data.sample_df.columns, index=data.get_col_pos(data.var_avg_bal), label_visibility="collapsed")
        
        with col4:
            st.markdown("###### MOB")
            data.mob = int(st.number_input("mob", label_visibility="collapsed", min_value=1, value=data.mob))
            
        with col5:
            st.markdown("###### Revenue")
            data.var_revenue = st.selectbox("revenue", options=data.sample_df.columns, index=data.get_col_pos(data.var_revenue), label_visibility="collapsed")
