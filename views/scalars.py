...
def annualizing_factor_calculation(scalar: Scalar, data: Data) -> Scalar:
    ...
    st.subheader(
        f"{'$' if scalar.loss_rate_type == 'dlr' else r'#'} Scalar Calculation"
    )
    ...
    col2.metric(
        label="{0} Portfolio Scalar".format(
            "$" if scalar.loss_rate_type == "dlr" else r"\#"
        ),
...
def risk_scalar_factor_calculation(scalar: Scalar) -> Scalar:
    df = pd.DataFrame(
        {
            ...
            Names.MATURITY_ADJUSTMENT_FACTOR.value: scalar.maturity_adjustment_factor
            * 100,
    ...
    df = df.style.map(
        lambda v: f"background-color: {color_map[v]};", subset=[Names.RISK_TIER.value]
    )
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            Names.RISK_SCALAR_FACTOR.value: st.column_config.NumberColumn(
                label=Names.RISK_SCALAR_FACTOR.value, format="%.2f", disabled=True
            ),
        },
        hide_index=True,
        use_container_width=True,
    ...
    if not edited_df.equals(df.data):
        scalar.maturity_adjustment_factor = (
            edited_df[Names.MATURITY_ADJUSTMENT_FACTOR.value] / 100
        )
